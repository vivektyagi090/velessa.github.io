...
import { ProductCategory, ProductCollection, MetalType, Product } from '../../types/product';
...
export const ProductFilters: React.FC<ProductFiltersProps> = ({
  ...
  const productList = useMemo(() => {
    return products && products.length > 0 ? products : productService.getInitialProductsSync();
  }, [products]);

  // Compute live, accurate product count for every category based on actual products
  ...
  const getCategoryCount = (catName: string): number => {
    ...
    // Flexible singular/plural check (e.g. "rings" vs "ring")
    const entry = Object.entries(categoryCounts).find(([k]) => {
      if (k === key) return true;
      if (k.endsWith('s') && k.slice(0, -1) === key) return true;
      if (key.endsWith('s') && key.slice(0, -1) === k) return true;
      return false;
    });
  ...
  const categoriesToDisplay = useMemo(() => {
    ...
    const dynamicList = [...CATEGORIES.map((c) => ({ id: c.id, name: c.name }))];
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 text-xs">
          ...
          {categoriesToDisplay.map((cat) => {
            ...
            const isChecked = filters.category.toLowerCase().trim() === cat.name.toLowerCase().trim();
            return (
              <label
                key={cat.id}
              ...
              >
                <span className="flex items-center gap-2.5">
                  <input
                    ...
                    name="category"
                    checked={isChecked}
                    onChange={() => handleCategoryChange(cat.name as any)}
                    className="accent-champagne"
                  />
                  <span>{cat.name}</span>
                </span>
                <span className="text-[10px] text-charcoal-muted">({count})</span>
              </label>
            );
          })}
        </div>
