...
export const HomePage: React.FC = () => {
  const [newArrivals, setNewArrivals] = useState<Product[]>(() => {
    const all = productService.getInitialProductsSync();
    return all.filter((p) => p.isNew || p.collection === 'Festive Collection').slice(0, 4);
  });
  const [bestSellers, setBestSellers] = useState<Product[]>(() => {
    const all = productService.getInitialProductsSync();
    return all.filter((p) => p.isBestSeller).slice(0, 4);
  });
  ...
  return (
    <div className="min-h-screen bg-ivory">
      {/* 1. Hero Section (Commented out as requested) */}
      {/* <HeroSection /> */}
