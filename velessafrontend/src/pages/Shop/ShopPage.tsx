...
export const ShopPage: React.FC = () => {
  ...
  const [allProducts, setAllProducts] = useState<Product[]>(() => productService.getInitialProductsSync());
  const [isInitialLoading, setIsInitialLoading] = useState<boolean>(() => productService.getInitialProductsSync().length === 0);
  ...
  // Fetch all products once on mount (instant if cached from another page)
  useEffect(() => {
    let isMounted = true;
    const loadAll = async () => {
      try {
  ...
  // Compute filtered products instantaneously (0ms) - eliminates skeleton flicker during slider drag or filter clicks
  const products = useMemo(() => {
    return productService.applyFilters(allProducts, filters);
  }, [allProducts, filters]);
